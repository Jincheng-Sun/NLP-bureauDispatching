...
def model_w2v(input,usebias=False,istraining=True):
    ...
    conv2 = conv_layer(pool1, 'conv2', 3, 3, 32, 16,usebias=usebias,istraining=istraining)
    #Layer 3：第二层pooling, outpit size[m,64,25,32]
    pool2 = tf.nn.max_pool(conv2,[1,4,2,1],[1,4,2,1],padding='SAME')
    #Flatten, output size [m,1,1,19*19*32]
    conv2 = tf.reshape(pool2,[-1,1,1,12400])
    #Layer 4：第一层全连接层, kernel [24800,256],output size [m, 256]
    fc1 = fc_layer(conv2, 'fc1', 128,usebias=usebias,istraining=istraining,dropout=0.5)
    #Layer 5：第二层全连接层, kernel [256,64], output size [m,64]
    fc2 = fc_layer(fc1, "fc2", 64,usebias=usebias,istraining=istraining,dropout=0.5)
    #Layer 6：第三层全连接层, kernel [64,n_cls], output size [m,n_cls], 不使用激活函数
    fc3 = fc_layer(fc2, 'fc3', n_cls,usebias=usebias,useactivation=False,istraining=istraining,dropout=0.5)
...
def dataset_input_fn(filenames,batch,buffersize,onehot=True,test=False):
...
def train():
    ...
    vec_batch, label_batch = dataset_input_fn('train36000v1.tfrecords', batch_size, 1000)
...
##############################
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True,
        ...
        saver2.restore(sess, './model2/model.ckpt-3599')
        sess.run(init)
        ...
        for i in range(iters):
            ...
            if (i > 0 and i % 10 == 0):
                writer.add_summary(MERGE, i+3600)
...
def test():
    ...
    init = tf.group(tf.global_variables_initializer(), tf.local_variables_initializer())
    vec_batch, label_batch = dataset_input_fn('test4000v1.tfrecords', 200, 1,test=True)
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess,'./model2/model.ckpt-3599')
        sess.run(init)
        #
        acc_sum=0
        for i in range(20):
...
def conti_train():
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True,
        ...
        for i in range(iters):
            ...
            _, LOSS, ACCURACY, MERGE, log = sess.run([update_op, loss, accuracy, merge, logits],
                                                     feed_dict={batch_x: v, batch_y: l})
            # print('真实值：'+str(l))
            # print('估计值：'+str(log))
